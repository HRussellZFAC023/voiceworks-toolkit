...
Real-time synced subtitles purpose-built for immersion-based Japanese study. Japanese text is displayed as the primary line with blurrable English translations underneath — hover or press B to reveal. Automatically loads LRC lyric files when available and integrates with live Whisper transcriptions for unstudied content. The last confirmed live caption remains visible through short listening and catch-up gaps instead of being replaced by status text. Chinese-language subtitles are auto-translated to Japanese via Google Translate so learners always see kana/kanji as the primary line. Playback speed controls and configurable lead-time let you study at your own pace.

#### Live Transcription — On-Device Speech-to-Text
Turns any voicework into study material by transcribing bounded live audio directly from the player element — no file uploads or duplicate full-track download on the normal path. [Transformers.js](https://huggingface.co/docs/transformers.js) runs multilingual Whisper in a dedicated worker on the exact backend and model the user selects. **Auto** resolves a device-appropriate model before a run begins; Firefox/M1 uses timestamped Base for stronger Japanese recognition and word-aligned karaoke, while CPU-only and constrained devices retain Tiny. An explicit Tiny, Base, Small, Medium, or Large v3 Turbo choice is never silently replaced after loading or inference starts. If live capture is unavailable, Whisper tries the host's lower-quality stream before a compatibility decode; a full stream is eligible only when its reported size is at most 32 MiB. Model loading, inference, streaming, and decode operations all have bounded recovery paths instead of stalling indefinitely. The worker keeps a bounded queue, reports playhead-local backlog plus measured real-time throughput, and only starts the stall watchdog when inference actually begins. Transcripts are cached per-track with a 90-day TTL and export to LRC, VTT, or raw untranslated source-language TXT. A labelled **Model Quality** control shows the model tiers and approximate download sizes; separate context and overlap controls default to ASMR-quality 29-second windows with 5-second overlap. **Adaptive live window** is enabled by default for Firefox/M1 Auto: it uses an 8-second foreground window when keeping up, widens toward the configured 29-second/5-second ASMR window only when measured throughput and local lag show that batching will help, and returns to the shorter window after catching up. It never changes the resolved model or backend. Generic speech VAD and silence skipping are disabled so quiet words, breaths, and long pauses always reach the model; only pathological runs of 12 or more identical short output segments are compacted.

**WebGPU compatibility:** the userscript probes WebGPU inside the transcription worker and reports a clear terminal error when a selected backend or model cannot run; it does not silently change either choice. Users can explicitly select WASM when CPU execution is desired. Firefox Stable enables WebGPU by default on Windows from Firefox 142 and Apple Silicon macOS from Firefox 147. Firefox on Linux or Intel macOS still depends on the browser channel and user configuration, so the runtime capability probe remains authoritative. The userscript cannot and does not change browser feature flags. See [Mozilla's current WebGPU rollout notes](https://developer.mozilla.org/en-US/docs/Mozilla/Firefox/Experimental_features#webgpu_api).
...
#### Player Fullscreen
CSS-based fullscreen expansion for the player area with an in-player toggle. Preserves playback during toggle and syncs with the Player Gallery when active. Keyboard shortcut: F.
...
The **Download works** action in the top toolbar opens immediately on **Site**, where multilingual semantic search is merged with the live full-site catalog. Results show work covers and calculate category-aware download sizes from the real track manifest in the background. Signed-in users can switch to **Yours**, while **Community** is backed by a cached server catalog; personal playlists remain authenticated and live. Select whole playlists or individual works and recreate each selected work's complete folder tree; every file category is included by default, with explicit opt-outs for storage-conscious downloads. Reversible legacy CP932 filename corruption is repaired only when the complete path segment can be decoded safely. Optional in-browser Opus conversion preserves source tags and artwork by default, adds missing metadata and `cover.jpg` safely, and supports an explicit overwrite mode; duration-aware rows estimate the converted size before downloading. Work download concurrency defaults to one and is configurable up to three parallel files, while Opus conversion stays serial to cap memory. Cross-origin CDN files never receive host credentials, full-quality stream URLs can replace rejected download URLs, and mid-body stalls retry from the exact durable byte offset. Common audio files checkpoint every 64 MiB; when the CDN exposes no ETag or modification date, resume verifies SHA-256 samples from the committed local prefix and boundary against the same canonical remote object before appending. Per-file checkpoints, discovery progress, options, and destination-folder handles are persisted, so interrupted downloads resume without repeating completed files.
...
```bash
npm test                # Vitest unit tests (1,912 tests across 173 files)
...
Set `E2E_PROXY_URL` for a private worker. Restricted runners that cannot bind a Vite port can add `E2E_SKIP_WEBSERVER=1`; the fixtures still inject the built userscript directly. The current unit baseline is 173 files / 1,912 tests.
...
- **Whisper Backend Policy** - Each run pins its selected WebGPU or WASM backend and model; mismatches fail visibly instead of substituting another choice
...
| GPU Acceleration | User-selected WebGPU or WASM execution |
