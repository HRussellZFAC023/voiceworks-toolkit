...
Turns any voicework into study material by transcribing bounded live audio directly from the player element — no file uploads or duplicate full-track download on the normal path. [Transformers.js](https://huggingface.co/docs/transformers.js) runs multilingual Whisper in a dedicated worker on the exact backend and model the user selects. **Auto** resolves a conservative device-appropriate model before a run begins; an explicit Tiny, Base, Small, Medium, or Large v3 Turbo choice is never silently replaced after loading or inference starts. If live capture is unavailable, Whisper tries the host's lower-quality stream before a compatibility decode; a full stream is eligible only when its reported size is at most 32 MiB. Model loading, inference, streaming, and decode operations all have bounded recovery paths instead of stalling indefinitely. The worker keeps one active and one replaceable queued window, and only starts the stall watchdog when inference actually begins. Transcripts are cached per-track with a 90-day TTL and export to LRC, VTT, or raw untranslated source-language TXT. A labelled **Model Quality** control shows the model tiers and approximate download sizes; separate context and overlap controls default to ASMR-quality 29-second windows with 5-second overlap. Generic speech VAD and silence skipping are disabled so quiet words, breaths, and long pauses always reach the model.
