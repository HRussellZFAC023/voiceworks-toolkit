...
Before choosing a destination folder, select which audio, video, image, text, and other files to include. Complete-work downloads include all five categories by default; uncheck any category you do not need to reduce storage use. File concurrency defaults to one and can be raised to three when more throughput is useful; Opus conversion remains serial to bound browser memory.
